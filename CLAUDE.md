...
3. If not immediately matched, a `@Scheduled` batch in `MatchingService.matching()` runs every 30s (only while `KisService.isMarketOpen()`), pulls all distinct stock codes with `PENDING`/`PARTIAL` orders, fetches each one's current price from KIS, and matches eligible orders in true price-then-time priority: `OrderRepository.findPendingBuyOrders`/`findPendingSellOrders` sort by `price DESC`/`price ASC` respectively (the more aggressive limit price goes first — higher for buys, lower for sells), then `createdAt ASC` as the tiebreaker. Despite the README calling this "Spring Batch," it is a plain `@Scheduled` method — there is no Spring Batch dependency in the project.
...
- Local log pipeline for this to work: `observability/docker-compose.yml` runs Loki + Grafana (`docker compose up -d` from that directory); the app pushes logs to Loki directly via a Logback appender (`logback-spring.xml`, `loki-logback-appender`) — no Promtail/file tailing involved. `observability/export-logs.sh` bulk-exports a time range via Loki's HTTP API for offline analysis.
- **`LOKI_URL` must be explicitly set to enable Loki logging — it no longer defaults to `localhost`.** `logback-spring.xml` wraps the `LOKI` appender (and its `<appender-ref>`) in `<if condition='!"".equals(property("LOKI_URL"))'>`, requiring `org.codehaus.janino:janino` on the runtime classpath. If `LOKI_URL` is unset, Loki logging is fully disabled instead of silently pointing at `localhost:3100`.
...
- **Before working around an external tool/library's behavior with custom code, check its official docs/search for an existing config option first.** Only fall back to a code-level workaround once you've confirmed no built-in option covers it. Precedent: the k6 session-cookie issue (see "http_req_failed 18%대" issue below) correctly diagnosed that k6 resets its cookie jar between VU iterations, but the fix was implemented as a manual workaround — storing the login cookie in a variable and attaching it as a `Cookie` header on every request — without first checking whether k6 already had a setting for this. It does: `options.noCookiesReset: true` keeps cookies across iterations natively. The manual workaround works, but it was solving with code a problem that had a one-line config fix (caught on 2026-08-25 re-review, not corrected in the script).
- **Never let an external-integration client/appender silently default to a plausible-looking endpoint (e.g. `localhost`) when its config var is unset.** Require explicit opt-in instead — if the var is missing, disable the integration, don't guess an address. Precedent: the Loki Logback appender used to default `LOKI_URL` to `http://localhost:3100/...`; on every EC2 deploy (where Loki isn't running) this caused infinite retry-with-full-stack-trace logging that filled the disk and, twice, corrupted the H2 `.mv.db` file mid-write — once taking down signup entirely and once corrupting Kafka/Zookeeper state alongside it. Fixed in `logback-spring.xml` by gating the appender behind `<if condition='!"".equals(property("LOKI_URL"))'>` (commit `2d3f752`). Apply the same skepticism to any future client for an external/optional service (metrics, tracing, webhooks, etc.) — an unset config var should mean "off," not "try localhost and retry forever."
- **When the user asks a question, answer that question first, directly.** If you're not sure what they're asking, ask before answering — do not guess. Never substitute an explanation, a list, or adjacent detail for the answer. When the user re-asks the same point, treat that as a signal you missed it: stop, give the direct answer, or ask. After answering, check it against what was actually asked and against the sources before sending. Precedent: in the matching-engine-heap writeup thread (2026-08-31) the assistant reframed the same conclusion four times and, when asked "무슨 비용이야?", kept producing multi-item breakdowns instead of the one-line answer ("개선 전 = 정렬로 인한 CPU 시간, 개선 후 = 힙 poll 비용"); the user had to spell out the answer themselves. Also logged as memory `feedback_grasp_question_then_verify_output`.
