...
# Issue (해결 안 됨 - 원인 재확인만 완료): 2000명 부하테스트 CPU 병목(2026-08-13, 미해결로 남겨뒀던 건) - BCrypt는 기여 요인이지만 단일 원인은 아님, 근본은 여전히 "로컬 8코어에 여러 CPU 소모 요인이 동시에 몰림"

## 발견 경위

기존 "2000명 지연시간 문제"(위 매칭엔진 Heap 재설계 이슈의 검증 섹션 참고)는 서버 CPU 96.7% 점유까지만 확인하고, 어느 코드 경로가 그 CPU를 쓰는지는 특정하지 못한 채 "로컬 환경(k6와 서버가 같은 노트북 CPU를 공유) 한계로 판단, 미해결"로 남겨뒀었음. 장 마감 후(2026-08-25 18시경) 이 원인을 실측하기 위해, `MatchingService`의 장마감 체크와 `KisWebSocketService`의 실시간 호가를 테스트 전용으로 임시 우회(장 시간 무관하게 매칭엔진 자체 로직은 그대로 재현, KIS 외부 연동만 합성 데이터로 대체)해서 500명 규모 축소 재현 + JFR 프로파일링을 진행함.

## 확인된 사실

- JFR로 202초간 실행 스택을 11,235회 샘플링한 결과, **`BCrypt.encipher`(비밀번호 해싱 내부 연산) 단일 메서드가 전체 샘플의 31.2%(3,504건)** 를 차지함. `BCrypt.key`까지 합치면 32%가 넘음 — 두 번째로 큰 항목군(H2 DB 내부 메서드들, `MVStore`/`Value` 관련)을 다 합쳐도 이보다 작고, `MatchingService`/`OrderBook` 관련 스택은 상위 25개 안에 아예 없었음.
- 서버 CPU는 부하 유지 구간에서 지속적으로 640~697%(8코어 중)로, 기존(2026-08-13) 96.7% 관측치와 같은 수준의 포화 상태를 재현함.
- `order_duration` p95 6.62초, `rank_duration`/`detail_duration` p95도 180~192ms로 크게 열화 — 원본 이슈가 "캐시만 읽는 가벼운 엔드포인트까지 같이 느려진 게 결정적 단서"라고 짚었던 현상과 일치. 다만 그 결정적 단서의 실제 원인이 매칭엔진이 아니라, **로그인 시 BCrypt 연산이 전역 CPU를 점유해서 다른 모든 요청(매칭 포함)이 그 여파로 같이 느려진 것**일 가능성이 새로 제기됨.
- `PasswordConfig`는 `new BCryptPasswordEncoder()`(기본 strength=10)를 그대로 씀 — 의도적으로 느리게 설계된 알고리즘이라, 수백 명이 짧은 시간에 몰려 로그인하면 CPU 비용이 그만큼 집중됨.

## 인과관계 실측 (2026-08-25, 같은 날 이어서 진행)

BCrypt가 진짜 원인인지 확인하려고, 테스트 계정(`loadtest_`/`fdsabuser_`, 2006개)의 비밀번호 해시만 strength 10 → 4로 재해싱(같은 비밀번호 `loadtest1234!`, 로그인 자체는 정상 동작 확인)한 뒤 **완전히 동일한 조건으로 재실행**함.

**결과: CPU/지연시간 둘 다 개선되지 않음.**

| 지표 | strength=10 (기존) | strength=4 (낮춤) |
|---|---|---|
| CPU 최대치(8코어=800%) | 697% | 692.5% |
| `order_duration` p95 | 6.62s | 7.96s (오히려 악화) |
| `rank_duration` p95 | 192.3ms | 280.1ms (오히려 악화) |
| `detail_duration` p95 | 180.4ms | 285.8ms (오히려 악화) |
| BCrypt가 실행샘플에서 차지하는 비중 | 31.2%(3504/11235) | **0%(상위 25위 밖으로 사라짐)** |

JFR로 재확인해보니 BCrypt 비중은 실제로 0에 가깝게 사라졌는데도(의도한 대로 낮아짐 확인), 정작 CPU 총량과 지연시간은 전혀 안 줄고 오히려 소폭 악화됨. 대신 그 자리를 H2 DB 내부 메서드들(`TransactionMap$CommittedIterator.fetchNext`, `Cursor.hasNext` 등)이 채웠음 — 1위였던 것 하나가 빠지니 2위군이 올라온 것.

## 결론

**BCrypt는 실제로 CPU를 많이 쓰는 요인 중 하나였던 건 맞지만(31%는 실측으로 확인된 사실), 그걸 없앤다고 전체 CPU/지연시간이 줄지는 않았다.** 즉 이 로컬 테스트 환경(k6와 서버가 노트북 8코어를 나눠 쓰는 구조)은 이미 여러 요인(BCrypt, H2 트랜잭션/MVStore 조회, GC - 202초 동안 Young GC 609회 발생, 매칭엔진의 건당 재조회 등)이 동시에 8코어를 다 채우고 있는 상태라, 그중 하나를 줄여도 나머지가 그 자리를 즉시 채워서 총량은 그대로였던 것으로 보임. **"CPU 자체가 부족해서 모든 요청이 골고루 대기하는 상황"이라는 2026-08-13의 원래 진단이, 특정 단일 원인이 아니라는 의미에서 오히려 더 정확했던 것으로 재확인됨.**

이번 실측으로 명확해진 것: 이 CPU 병목은 코드 레벨의 단일 핫스팟 최적화로 풀리는 문제가 아니라, **테스트 환경 자체(부하생성기+서버 co-location, 8코어 한계)의 구조적 제약**일 가능성이 높음. 근본적으로 확인하려면 이전에 시도했다가 인스턴스 스펙 부족으로 중단됐던 "서버-클라이언트 분리 측정"을 더 큰 인스턴스로 재시도해야 함 — 이번 조사에서도 그 결론은 바뀌지 않음.

## 진단 과정에서 확인한 것 / 주의사항

- **비밀번호 강도를 낮추는 건 이번 실측 목적의 테스트 계정(`loadtest_`/`fdsabuser_`)에 한정한 일회성 진단 조치였고, 실험 종료 후 즉시 strength=10으로 재해싱해서 원복함.** 프로덕션 `PasswordConfig`(코드)는 애초에 건드리지 않았고, 실사용자 계정도 전혀 손대지 않음.
- 이번 축소 재현은 로그인 램프업을 원본(2분)보다 압축(20초)한 버전이었음 — 다만 초당 로그인 시도량 자체는 원본(2000명/2분 ≈ 16.7건/초)보다 이번(500명/20초=25건/초)이 더 높아서, 결과를 과소평가할 방향의 차이는 아니라고 봄.
- 이번 재현 과정에서 부수적으로 확인: k6 스크립트의 로그인 성공 체크(`auth.js:21`, `r.status === 302`)가 **로그인 실패(→`/login?error`)와 로그인 성공(→`/stockHome`)을 구분하지 못함** — 둘 다 302라서 체크를 통과함. 이번 조사 중 새 프리픽스(`cputest`)로 테스트하다가 기존에 알려진 `phoneFor()` 전화번호 충돌 버그(위 항목 참고, 아직 미수정)로 전원 회원가입이 실패했는데, 이 허술한 체크 때문에 "로그인 성공"으로 잘못 표시되면서 원인 파악이 한 단계 더 걸림 — 아직 수정 안 함, 별도 이슈로 남겨둠.

## 실제 체결 속도까지 확인 (같은 날 이어서)

CPU/HTTP 응답시간 말고 "진짜 체결까지 걸린 시간"(`Trade.matched_at - Order.created_at`)도 두 조건에서 직접 계산해서 비교함.

- 즉시체결(`tryMatch`, 2초 이내에 체결된 것만) 속도: strength=10일 때 p50 9ms/p95 781ms → strength=4일 때 p50 690ms(76배 느려짐)/p95 1,826ms(2.3배 느려짐). **BCrypt를 낮췄는데 즉시체결 속도도 오히려 나빠짐** — CPU가 이미 포화 상태라 BCrypt 자리가 비면 다른 스레드(H2 처리)가 그 자리를 채우면서, 매칭엔진 스레드(Kafka 리스너)가 오히려 CPU를 덜 배정받은 것으로 보임.
- 다만 이 절대값 자체는 신뢰도가 낮음 — 체결된 주문의 96%(strength=10: 5394/5627, strength=4: 4056/4208)가 즉시체결이 아니라 30초 배치를 여러 번 거쳐야 했는데, 이는 이번 실험에 쓴 합성 호가(가격 몇 단계 고정값)가 지정가 매수의 5% 가격개선 밴드에 잘 안 걸려서 생긴 인위적 현상. 실제 KIS 시세(촘촘한 가격대)였다면 즉시체결 비율이 훨씬 높았을 것 — 즉 "즉시체결 성공률 4%"라는 숫자는 매칭엔진 성능이 아니라 테스트 데이터의 한계.

## 다음 가설과 실제 수정 - 매칭엔진의 건당 DB 재조회(N+1) 개선

BCrypt를 낮추는 방향(진단만 하고 프로덕션엔 적용 불가한 조치)이 막힌 뒤, 사용자와 상의해서 실제로 프로덕션에 적용 가능한 개선 후보를 다시 검토함. 후보로 "로그인 레이트리밋"도 나왔으나 **주식 매매 시스템에서 로그인 자체를 막는 건 쇼핑몰 대기열/예약 시스템과는 성격이 다르다(장 시작 순간 접속을 막으면 안 됨)** 는 이유로 기각하고, 코드 레벨에서 실제 DB 부하를 줄이는 쪽으로 확정함.

JFR 프로파일에서 BCrypt 다음으로 컸던 게 H2 MVStore/트랜잭션 내부 메서드들(`TransactionMap$CommittedIterator.fetchNext`, `Cursor.hasNext` 등)이었고, 이건 `docs/matching-engine-heap-refactor.md`에 이미 기록돼 있던 문제와 정확히 같은 지점이었음: `MatchingService.drainAndExecute()`가 우선순위 큐에서 주문을 하나씩 꺼낼 때마다 `orderRepository.findById()`로 DB를 개별 재조회함 — 대기 주문이 N건이면 배치 사이클마다 DB 왕복이 N번.

**수정 (`MatchingService.java`)**: 큐에서 전부 꺼낸 뒤(우선순위 순서는 그대로 유지하기 위해 리스트로 보관), `orderRepository.findAllById()`로 한 번에 배치 조회하도록 변경. 체결 로직(가격-시간 우선순위, 레벨별 잔량 차감)은 그대로 두고 "DB에서 최신 상태를 어떻게 가져오는가"만 N번 개별 조회 → 1번 배치 조회로 바꿈.

**테스트 수정**: 기존 `MatchingServiceTest`의 5번(가격 우선순위)·8번(취소 반영) 테스트가 `orderRepository.findById(id)`를 개별 스텁하고 있었는데, 프로덕션 코드가 더 이상 이 메서드를 안 부르게 되면서 실패함(스텁 안 걸림 → 빈 결과) — `findAllById(any())`를 스텁하도록 수정 후 8개 테스트 전부 통과 확인(`tryMatch` 경로를 쓰는 나머지 테스트는 원래도 `findById` 그대로 사용하므로 영향 없음).

## 재측정 결과 (같은 날 이어서, findAllById 수정 적용 후)

같은 조건(500명, 합성 호가, 장마감 우회)으로 수정 후 코드를 다시 재현 + JFR 재프로파일링함(비밀번호 강도는 이번엔 건드리지 않고 기본값 그대로 둠 - 순수하게 이 수정 하나의 효과만 보기 위함).

**GC(가비지 컬렉션) 부담은 확실히 줄었음**:

| 지표 | 수정 전(findById N번) | 수정 후(findAllById 1번) |
|---|---|---|
| Young GC 횟수 | 609회 | 496회 (-18.6%) |
| Old GC 횟수 | 41회 | **7회 (-83%)** |
| iterations/s (처리량) | 112.17/s | 120.40/s (+7.3%) |

배치조회로 개별 엔티티 조회·H2 Value 래핑 객체 생성이 줄어드니 가비지가 덜 나오고, 특히 비용이 큰 Old GC가 크게 줄어든 것으로 보임. 처리량도 소폭 늘었음(다만 이 정도 폭은 이번 세션에서 반복 관찰된 런간 변동 범위 안이라 확정적 신호로 보기엔 조심스러움).

**반면 CPU 총량/응답시간 p95는 이번에도 뚜렷하게 개선되지 않음** — `order_duration` p95 6.62s→7.69s, `rank_duration` p95 192ms→280ms 등. 이유는 이번 재측정에선 BCrypt를 원래 강도(10)로 되돌려둔 채 진행해서, BCrypt가 여전히 실행샘플의 31.0%(3842/12410, 위 BCrypt 실측 때와 거의 동일한 비중)를 차지하며 CPU를 포화시키고 있었기 때문. 즉 이 로컬 환경에서는 BCrypt가 여전히 지배적인 단일 요인이라, 그 아래 있는 DB 쿼리 최적화 효과가 최종 사용자 체감 지연시간에까지는 잘 안 드러남.

## 최종 결론

- **`findAllById` 배치조회 수정 자체는 유효한 개선임** — GC 부담(특히 Old GC -83%) 감소로 실측 확인됨. 코드 품질/확장성 관점에서도 N+1 쿼리 패턴을 없앤 정당한 리팩터링이고, 회귀 테스트 8개 전부 통과함.
- 다만 **"체감 응답속도가 빨라졌다"는 수준까지는 이번 로컬 8코어 테스트 환경에서 증명하지 못함** — BCrypt가 여전히 CPU의 1/3을 쥐고 있는 상태라, 그 밑단 최적화 효과가 최종 지연시간까지 전파되지 않음(위 BCrypt 실측 때와 같은 "CPU가 이미 포화된 시스템에서는 한 곳을 줄여도 전체가 안 빨라진다" 패턴의 반복).
- 결국 이 CPU 병목을 실사용자 체감 속도로 확실히 개선하려면, 이번에 확인한 개별 최적화(DB 배치조회)들을 계속 쌓아가는 것과 별개로, **부하생성기와 서버가 CPU를 나눠 쓰지 않는 환경(서버-클라이언트 분리)에서 재측정**해야 진짜 효과를 볼 수 있음 — 2026-08-13에 시도했다가 인스턴스 스펙 부족으로 중단된 부분과 같은 결론.
